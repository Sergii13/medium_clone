...
const popularTags = computed(() => {
   return store.state.popularTags.data?.filter(tag => tag.length > 0)
})
...
<template>
   <div class="sidebar">
      ...
      <div v-if="popularTags" class="tag-list">
         <router-link v-for="popularTag in popularTags" :key="popularTag"
            :to="{ name: 'tag', params: { slug: popularTag } }" class="tag-default tag-pill ng-binding ng-scope">
            {{ popularTag }}
         </router-link>
      </div>
